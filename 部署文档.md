...
2. 向下滚动找到 "R2 Bindings" 部分（在 "Environment Variables" 附近）
3. 点击 "Add binding"
4. 输入以下信息：
...
**重要说明**：请注意，`wrangler.toml` 文件中没有预设的 bucket 名称，这是为了确保部署时不会因为无效的 bucket 名称而失败。您需要在部署完成后通过 Cloudflare 仪表板配置 R2 绑定。
